...
import { getCountryByName } from '@shared/api/config'
import { type Country, type CountrySlice } from '@shared/types/countries'
...
>(
  ...
  async (
    countryName,
    { dispatch, rejectWithValue, extra: { client } }
  ) => {
    try {
      return (await client.get(getCountryByName(countryName))).data[0]
    } catch (error) {
